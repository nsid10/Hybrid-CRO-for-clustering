...
class Molecule:
    def __init__(self, n_cluster, data, KE, use_kmeans=True):
        if use_kmeans:
            kmeans = KMeans(n_clusters=n_cluster, max_iter=60)
            kmeans.fit(data)
...
class CRO:
    ...
    def _init_molecules(self):
        ...
        for i in range(self.n_molecules):
            if i == 0 and self.KMCRO:
                molecule = Molecule(self.n_clusters, self.data, self.baseKE, use_kmeans=True)
            else:
                molecule = Molecule(self.n_clusters, self.data, self.baseKE, use_kmeans=False)
    ...
    def _synthesis(self, molecule_1, molecule_2):
        ...
        new_molecule.update(self.data, True)
    ...
    def _intermolecular_ineffective_collision(self, molecule_1, molecule_2):
        # two-exchange operation on each molecule
        self.iic += 1
        molecule_1.NHits += 1
        molecule_2.NHits += 1
    ...
    def _decomposition(self, molecule):
        ...
        new_molecule[0].update(self.data, True)
        new_molecule[1].update(self.data, True)
    ...
    def _onwall_ineffective_collision(self, molecule):
        # two-exchange operation on molecule
        self.oic += 1
        molecule.NHits += 1
